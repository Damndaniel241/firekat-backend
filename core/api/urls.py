...
urlpatterns = [
    ...
    path('search/all/',GetTopicsAndComments.as_view()),
...
# Given the CommentViewSet and the router registration for comments, Django Rest Framework (DRF) will automatically generate several URL patterns for the Comment model. Here's a list of the URLs that would be generated based on the provided setup:

# List of URLs for Comment:
# List all comments:
# GET /api/comments/
...
# Retrieves a specific comment by its ID (e.g., /api/comments/1/).

# Update a specific comment by ID (full update):
# PUT /api/comments/{comment_id}/
# Replaces the entire comment with new data (requires authentication).

# Update a specific comment by ID (partial update):
# PATCH /api/comments/{comment_id}/
# Partially updates fields of a specific comment (requires authentication).

# Delete a specific comment by ID:
# DELETE /api/comments/{comment_id}/
# Deletes a specific comment (requires authentication).

# These are the standard CRUD operations generated by DRF when using a ModelViewSet and registering it with a router. The get_permissions method ensures that the list and retrieve actions are open to everyone (AllowAny), while creating, updating, and deleting require the user to be authenticated (IsAuthenticated).